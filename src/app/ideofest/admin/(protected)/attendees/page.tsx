  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black">Attendee Management</h1>
          <p className="text-white/40 text-xs sm:text-sm mt-1">{totalTickets} total tickets across {bookings.length} passes in database</p>
        </div>

        <button
          onClick={handleExportCsv}
          className="flex items-center justify-center gap-2 bg-[#c1e527] hover:bg-[#b0d420] text-section-ink px-4 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all shadow-lg"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-[#c1e527]/10 border border-[#c1e527]/30 rounded-2xl p-5 backdrop-blur-md">
          <p className="text-xs font-bold text-[#c1e527] uppercase tracking-widest">Confirmed & Checked In</p>
          <p className="text-2xl sm:text-3xl font-black text-[#c1e527] mt-1">{confirmedBookings.reduce((s, b) => s + (b.quantity || 1), 0)}</p>
          <p className="text-xs text-[#c1e527]/70 mt-1">{confirmedBookings.length} active passes</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder="Search attendee name, email, phone, NIC..."
        ...
        <div className="flex rounded-xl border border-white/12 overflow-hidden bg-white/5 shrink-0">
          {(['all', 'confirmed', 'pending_verification'] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilterStatus(f)}
              className={`px-3 py-2 sm:px-4 sm:py-2.5 text-xs font-bold capitalize transition-colors ${
                    ...
                    <tr key={a.id || a.booking_ref} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                      <td className="px-5 py-4">
                        ...
                        <div className="flex items-center gap-3 mt-1 text-[11px] text-white/40 font-mono">
                          {a.nic_number && <span>NIC: {a.nic_number}</span>}
