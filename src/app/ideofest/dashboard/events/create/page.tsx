...
const TIERS_DEFAULT: TierForm[] = [
  { name: 'standard', label: 'Standard Pass', price: '1500', capacity: '150', perks: 'General Admission\nEvent Entry' },
];
...
export default function CreateEventPage() {
  ...
  const [bankDetails, setBankDetails] = useState({
    ...
    bankAccountName: 'Ideomint Organizer Account',
  ...
  const addTier = () =>
    setTiers([...tiers, { name: 'early_bird', label: 'Early Bird Pass', price: '1000', capacity: '50', perks: 'Early Bird Discount\nPriority Entry' }]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If file selected, upload via /api/ideofest/upload first
      if (imageFile) {
      ...
      setDone(true);
      setTimeout(() => router.push('/ideofest/dashboard/events'), 2000);
    } catch (e) {
      console.error(e);
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center">
        <div className="w-16 h-16 rounded-full bg-creative-flame/20 flex items-center justify-center mb-4 border border-creative-flame/40">
          <Check className="w-8 h-8 text-creative-flame" />
        </div>
        <h2 className="text-2xl font-black">Event Created!</h2>
        <p className="text-white/50 mt-2">Redirecting to events manager…</p>
      </div>
    );
  }

  const inputClass =
    'w-full bg-white/5 border border-white/12 focus:border-creative-flame rounded-xl px-4 py-3 text-white placeholder-white/30 focus:outline-none transition-colors';
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-8">
        ...
        <p className="text-white/40 text-sm mt-1">Step {step} of 3 — Fill in basic info, ticket tiers & payment options</p>
      </div>

      {/* Progress */}
      <div className="flex gap-2 mb-8">
        {[1, 2, 3].map((s) => (
          <div key={s} className={`h-1 flex-1 rounded-full transition-all ${step >= s ? 'bg-creative-flame' : 'bg-white/10'}`} />
        ))}
      </div>
        ...
        <div className="flex flex-col gap-5">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Ideomint Live Vol. 02"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input
              ...
              className={inputClass}
              placeholder="e.g. Viharamahadevi Amphitheatre"
            />
          </div>
          ...
          <div className="flex items-center gap-3 bg-white/5 border border-white/10 p-4 rounded-xl">
            <input
              type="checkbox"
              id="guestBookingDashboard"
              checked={form.guestBookingAllowed}
              onChange={(e) => setForm({ ...form, guestBookingAllowed: e.target.checked })}
              className="w-4 h-4 accent-creative-flame"
            />
            <label htmlFor="guestBookingDashboard" className="text-sm font-semibold cursor-pointer">
              Allow guest bookings without requiring account registration
            </label>
          </div>
          ...
          <button
            ...
            className="w-full mt-2 py-4 bg-creative-flame hover:bg-[#E54D30] disabled:opacity-40 text-white font-black rounded-xl transition-colors"
          >
            Next: Tickets & Payment Options →
          </button>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="flex flex-col gap-6">
          <div>
            ...
            <button
              onClick={addTier}
              className="w-full py-3 border border-dashed border-white/20 rounded-xl text-white/60 hover:text-white hover:border-creative-flame/50 transition-colors text-sm font-bold flex items-center justify-center gap-2"
            >
              <Sparkles className="w-4 h-4 text-creative-flame" /> + Add Ticket Tier
            </button>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <CreditCard className="w-5 h-5 text-creative-flame" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
              <label
                className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${
                  paymentMethods.bank_transfer
                    ? 'bg-creative-flame/10 border-creative-flame text-white'
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40">Slip upload verification</p>
                </div>
              </label>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40">Credit / Debit gateway</p>
                </div>
              </label>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40">Free registration</p>
                </div>
              </label>
            </div>
            ...
            {paymentMethods.bank_transfer && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-3 text-white/70 text-sm font-bold">
                  <Building2 className="w-4 h-4 text-creative-flame" /> Bank Account Details
                </div>
      ...
      {step === 3 && (
        <div className="flex flex-col gap-6">
              ...
              {imagePreview ? (
                <div className="relative w-full aspect-video rounded-2xl overflow-hidden border border-white/12">
                  ...
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                    <p className="text-white font-semibold text-sm">Click to change banner image</p>
                  </div>
                </div>
              ) : (
          ...
          <div className="flex flex-col gap-2">
            <label className={labelClass}>Or Enter Direct Image URL</label>
            <input
              type="url"
              value={imageUrlInput}
